    ...
    S: Send,
{
    fn send(&mut self, arg: &T) -> Option<RecycledFuture<'_, Result<R, SendError>>> {
        (self.query_filter_map)(arg).map(|arg| {
            ...
            // The previous future generated by this method should have been
            // polled to completion so a new sender should be readily available.
            let reply_sender = reply_receiver.sender().unwrap();
            ...
            RecycledFuture::new(fut_storage, async move {
                ...
                // Wait until the message is processed and the reply is sent
                // back. If an error is received, it most likely means the
                // mailbox was dropped before the message was processed.
                reply_receiver
                    .recv()
                    .await
                    .map_err(|_| SendError)
