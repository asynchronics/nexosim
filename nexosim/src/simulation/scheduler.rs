...
//     /// If this is a periodic action, returns a boxed clone of this action
// and     /// its repetition period; otherwise returns `None`.
...
//     pub(crate) fn into_future(self) -> Pin<Box<dyn Future<Output = ()> +
// Send>> {         self.inner.into_future()
//     }

//     /// Spawns the future that performs the action onto the provided
// executor.     ///
//     /// This method is typically more efficient that spawning the boxed
// future     /// from `into_future` since it can directly spawn the unboxed
// future.     pub(crate) fn spawn_and_forget(self, executor: &Executor) {
