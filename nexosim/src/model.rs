...
//! for any object implementing the [`Model`] trait, when its [`Model::Env`]
//! type is a unit type `()`. In such case the associated [`ProtoModel::Model`]
//! type is the model type itself. This is what makes it possible to use either
//! an explicitly-defined [`ProtoModel`] as argument to
