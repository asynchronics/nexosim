...
pub use injector::{EventInjector, Injector};
...
impl Simulation {
    ...
    /// Internal helper attempting to execute next scheduled events or ticks.
    ///
    /// Returns true if at least one event has been processed.
    // fn handle_scheduler_step(
    //     &mut self,
    //     upper_time_bound: MonotonicTime,
    // ) -> Result<Option<(MonotonicTime, bool)>, ExecutionError> {
    //     // Set to simulation time to the next scheduled event or next tick,
    //     // whichever is earlier.
    //     let next_tick = self.ticker.as_mut().and_then(|ticker| {
    //         let tick = ticker.next_tick(self.time.read());
    ...
    //     let mut next_key = peek_next_key(&mut scheduler_queue, upper_time_bound);
    //     let time = match (next_key, next_tick) {
    //         (Some(key), Some(tick)) => tick.min(key.0),
    //         (Some(key), None) => key.0,
    //         (None, Some(tick)) => tick,
    ...
    //             let key = peek_next_key(&mut scheduler_queue, upper_time_bound);
    //             if key != next_key {
    //                 break key;
    //             }
    //         };
