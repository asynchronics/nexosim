            ...
            .extend(visitor.type_params_for_bound.into_iter().map(|ty| {
                syn::WherePredicate::Type(syn::PredicateType {
                    ...
                    bounds: Punctuated::from_iter([syn::TypeParamBound::Trait(syn::TraitBound {
                        ...
                        lifetimes: None,
                        path: parse_quote!(nexosim::Message),
                    })]),
                })
            }));
    }
    ...
    fn visit_field(&mut self, field: &Field) {
        match &field.attrs.with {
            ...
            Some(WithAttr::Function(_)) => {
                // `schema_with` function type params may or may not implement
                // `Message`
            }
    ...
    fn visit_path(&mut self, field: &Field, path: &syn::Path) {
        if let Some(seg) = path.segments.last() {
            if seg.ident == "PhantomData" {
                ...
                // Message whether or not T implements it.
                return;
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_enum_bounds() {
        // All type params should be included in `Message` trait bounds except `Z`
        let input = parse_quote! {
            #[schema(rename = "MyEnum<{T}, {U}, {V}, {W}, {X}, {Y}, {{Z}}>")]
