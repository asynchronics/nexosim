...
pub struct SchemaExpr {
    /// Definitions for types or functions that may be used within the creator
    /// or mutators
    definitions: Vec<TokenStream>,
    /// An expression that produces a `Schema`
    creator: TokenStream,
    /// Statements (including terminating semicolon) that mutate a var `schema`
    /// of type `Schema`
    ...
    let mut schema_expr = if let Some(with) = &cont.attrs.with {
        ...
        quote! {
            if #GENERATOR.contract().is_deserialize() {
                <#from as nexosim::Message>::json_schema(#GENERATOR)
            } else {
                <#into as nexosim::Message>::json_schema(#GENERATOR)
            }
        }
        .into()
    } else {
...
pub fn expr_for_repr(cont: &Container) -> Result<SchemaExpr, syn::Error> {
    let repr_type = cont.attrs.repr.as_ref().ok_or_else(|| {
        syn::Error::new(
            Span::call_site(),
            "Message_repr: missing #[repr(...)] attribute",
        )
    })?;
    ...
    let Data::Enum(variants) = &cont.data else {
        return Err(syn::Error::new(
            Span::call_site(),
            "Message_repr can only be used on enums",
        ));
    };
    ...
    if let Some(non_unit_error) = variants.iter().find_map(|v| match v.style {
        ...
        _ => Some(syn::Error::new_spanned(
            v.original,
            "Message_repr: must be a unit variant",
        )),
    }) {
    ...
    let mut schema_expr = SchemaExpr::from(quote!({
        let mut map = nexosim::schema::_private::serde_json::Map::new();
        map.insert("type".into(), "integer".into());
        map.insert(
            "enum".into(),
            nexosim::schema::_private::serde_json::Value::Array({
                let mut enum_values = nexosim::schema::_private::alloc::vec::Vec::new();
                #(enum_values.push((#enum_ident::#variant_idents as #repr_type).into());)*
                enum_values
            }),
        );
        nexosim::schema::Schema::from(map)
    }));
...
fn expr_for_container_with(cont: &Container, with_attr: &WithAttr) -> SchemaExpr {
    ...
    let mut schema_expr = SchemaExpr::from(quote! {
        <#ty as nexosim::Message>::json_schema(#GENERATOR)
    });
...
fn expr_for_field(
    ...
    let schema_expr = if field.attrs.validation.required {
        quote_spanned! {span=>
            <#ty as nexosim::Message>::_schemars_private_non_optional_json_schema(#GENERATOR)
        }
    } else if is_internal_tagged_enum_newtype {
        quote_spanned! {span=>
            nexosim::schema::_private::json_schema_for_internally_tagged_enum_newtype_variant::<#ty>(#GENERATOR)
        }
    } else {
        quote_spanned! {span=>
    ...
    match with_attr {
        ...
        WithAttr::Function(fun) => {
            ...
            let type_def = quote_spanned! {fun.span()=>
                ...
                impl #impl_generics nexosim::Message for _SchemarsSchemaWithFunction<#cont_name #ty_generics> #where_clause {
                    fn inline_schema() -> bool {
                        true
                    }

                    fn schema_name() -> nexosim::schema::_private::alloc::borrow::Cow<'static, str> {
                        nexosim::schema::_private::alloc::borrow::Cow::Borrowed(#fn_name)
                    }

                    fn schema_id() -> nexosim::schema::_private::alloc::borrow::Cow<'static, str> {
                        nexosim::schema::_private::alloc::borrow::Cow::Borrowed(::core::concat!(
                    ...
                    fn json_schema(generator: &mut nexosim::schema::SchemaGenerator) -> nexosim::schema::Schema {
                        #fun(generator)
                    }
                }
            };
...
fn expr_for_enum(
    ...
    if variants.is_empty() {
        return quote!(nexosim::schema::Schema::from(false)).into();
    }
    ...
    schemas.extend(complex_variants.into_iter().map(|variant| {
        ...
        let mut schema_expr =
            SchemaExpr::from(if variant.is_unit() && variant.attrs.with.is_none() {
                quote! {
                    nexosim::schema::_private::new_unit_enum_variant(#name)
                }
            } else {
                let sub_schema =
                    expr_for_untagged_enum_variant(cont, variant, deny_unknown_fields, false);
                quote! {
                    nexosim::schema::_private::new_externally_tagged_enum_variant(#name, #sub_schema)
                }
            });
    ...
    let variant_schemas = variants
        .map(|variant| {
            ...
            schema_expr.mutators.push(quote!(
                nexosim::schema::_private::apply_internal_enum_variant_tag(&mut #SCHEMA, #tag_name, #name, #deny_unknown_fields);
            ));
...
fn expr_for_adjacent_tagged_enum<'a>(
    ...
    let schemas = variants
        .map(|variant| {
            ...
            let mut outer_schema = SchemaExpr::from(quote!(nexosim::schema::json_schema!({
                "type": "object",
                "properties": {
                    #tag_name: (#tag_schema),
...
/// Callers must determine if all subschemas are mutually exclusive. The current
/// behaviour is to assume that variants are mutually exclusive except for
/// untagged enums.
...
fn enum_ref_variants(cont: &Container, variant: Option<&Variant>, expr: SchemaExpr) -> SchemaExpr {
    ...
    let type_def = quote! {
        ...
        impl #impl_generics nexosim::Message for _SchemarsRefVariant<#cont_name #ty_generics> #where_clause {
            fn inline_schema() -> bool {
                false
            }

            fn schema_name() -> nexosim::schema::_private::alloc::borrow::Cow<'static, str> {
                nexosim::schema::_private::alloc::borrow::Cow::Borrowed(#variant_name)
            }

            fn schema_id() -> nexosim::schema::_private::alloc::borrow::Cow<'static, str> {
                nexosim::schema::_private::alloc::borrow::Cow::Owned(
                    nexosim::schema::_private::alloc::format!(
                        "_SchemarsRefVariant/{}::{}",
                        <#cont_name #ty_generics as nexosim::Message>::schema_id(),
                        #variant_name,
                ))
            }
...
// This function is also used for tagged variants, in which case the resulting
// SchemaExpr will be embedded or mutated to include the tag.
// `is_actually_untagged` will be true if the enum or the variant has the
// `untagged` attribute.
...
fn expr_for_tuple_struct(cont: &Container, fields: &[Field]) -> SchemaExpr {
    ...
    quote!({
        let mut prefix_items = nexosim::schema::_private::alloc::vec::Vec::new();
        #(#fields)*
        let len = nexosim::schema::_private::serde_json::Value::from(prefix_items.len());
...
fn expr_for_struct(
        ...
        .map(|field| {
            if field.serde_attrs.flatten() {
                ...
                let mut schema_expr = SchemaExpr::from(quote_spanned! {ty.span()=>
                    nexosim::schema::_private::json_schema_for_flatten::<#ty>(#GENERATOR, #required)
                });

                schema_expr.definitions.extend(type_def);

                field.with_contract_check(quote! {
                    nexosim::schema::_private::flatten(&mut #SCHEMA, #schema_expr);
                })
            } else {
                ...
                if let Some(default) = field_default_expr(field, set_container_default.is_some()) {
                    schema_expr.mutators.push(quote! {
                        #default.and_then(|d| nexosim::schema::_schemars_maybe_to_value!(d))
                ...
                let is_optional = if has_skip_serialize_if && has_default {
                    ...
                    quote!(#GENERATOR.contract().is_deserialize() && <#ty as nexosim::Message>::_schemars_private_is_option())
                } else {
                    quote!(if #GENERATOR.contract().is_deserialize() {
                        #has_default || (!#required_attr && <#ty as nexosim::Message>::_schemars_private_is_option())
                    } else {
                ...
                field.with_contract_check(quote!({
                    #(#definitions)*
                    nexosim::schema::_private::insert_object_property(&mut #SCHEMA, #name, #is_optional, #schema_expr);
                }))
            }
        })
        .collect();
    ...
    SchemaExpr {
        ...
        creator: quote!(nexosim::schema::json_schema!({
            "type": "object",
            #set_additional_properties
        })),
        mutators: properties,
