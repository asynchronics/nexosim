...
impl ValidationAttrs {
    ...
    fn add_mutators2(&self, mutators: &mut Vec<TokenStream>, mut_ref_schema: &TokenStream) {
        ...
        if let Some(regex) = self.regex.as_ref().or(self.pattern.as_ref()) {
            mutators.push(quote! {
                nexosim::schema::_private::insert_validation_property(#mut_ref_schema, "string", "pattern", (#regex).to_string());
            });
        }

        if let Some(contains) = &self.contains {
            mutators.push(quote! {
                nexosim::schema::_private::must_contain(#mut_ref_schema, &#contains.to_string());
            });
        }
        ...
        if let Some(inner) = &self.inner {
            ...
            if !inner_mutators.is_empty() {
                mutators.push(quote! {
                    nexosim::schema::_private::apply_inner_validation(#mut_ref_schema, |inner_schema| { #(#inner_mutators)* });
                });
            }
        }
    }
    ...
    fn add_length_or_range(
        ...
        if let Some(min) = value.min.as_ref().or(value.equal.as_ref()) {
            ...
            mutators.push(quote!{
                nexosim::schema::_private::insert_validation_property(#mut_ref_schema, #required_format, #key, #min);
            });
        }
        ...
        if let Some(max) = value.max.as_ref().or(value.equal.as_ref()) {
            ...
            mutators.push(quote!{
                nexosim::schema::_private::insert_validation_property(#mut_ref_schema, #required_format, #key, #max);
            });
        }
    }
