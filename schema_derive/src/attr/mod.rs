    ...
    pub fn add_mutators(&self, mutators: &mut Vec<TokenStream>) {
        ...
        if let Some(doc) = &self.doc {
            title.get_or_insert_with(|| {
                quote!({
                    const TITLE: &str = nexosim::schema::_private::get_title_and_description(#doc).0;
                    TITLE
                })
            });
            description.get_or_insert_with(|| {
                quote!({
                    const DESCRIPTION: &str = nexosim::schema::_private::get_title_and_description(#doc).1;
                    DESCRIPTION
                })
            });
        }
        if let Some(title) = title {
            mutators.push(quote! {
                nexosim::schema::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "title", #title);
            });
        }
        if let Some(description) = description {
            mutators.push(quote! {
                nexosim::schema::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "description", #description);
            });
        }
        ...
        if !self.examples.is_empty() {
            let examples = self.examples.iter().map(|eg| {
                quote! {
                    nexosim::schema::_private::serde_json::value::to_value(#eg)
                }
            });
            mutators.push(quote! {
                #SCHEMA.insert("examples".into(), nexosim::schema::_private::serde_json::Value::Array([#(#examples),*].into_iter().flatten().collect()));
            });
        }

        for (k, v) in &self.extensions {
            mutators.push(quote! {
                #SCHEMA.insert(#k.into(), nexosim::schema::_private::serde_json::json!(#v));
            });
        }

        for transform in &self.transforms {
            mutators.push(quote_spanned! {transform.span()=>
                nexosim::schema::transform::Transform::transform(&mut #transform, &mut #SCHEMA);
            });
        }
    }
}
...
impl FieldAttrs {
    ...
    fn populate(&mut self, attrs: &[Attribute], cx: &Ctxt) {
        let schemars_cx = &mut AttrCtxt::new(cx, attrs, "schema");
    ...
    fn process_meta(
        ...
        match meta_name {
            ...
            "schema_with" if cx.attr_type == "schema" => match self.with {
...
impl ContainerAttrs {
    ...
    fn process_meta(
        ...
        match meta_name {
            ...
            "rename" if cx.attr_type == "schema" => match self.rename_format_string {
                Some(_) => cx.duplicate_error(&meta),
            ...
            "with" if cx.attr_type == "schema" => match self.with {
                Some(WithAttr::Type(_)) => cx.duplicate_error(&meta),
                Some(WithAttr::Function(_)) => cx.mutual_exclusive_error(&meta, "schema_with"),
                None => self.with = parse_name_value_lit_str(meta, cx).ok().map(WithAttr::Type),
            },
            ...
            "_unstable_ref_variants" if cx.attr_type == "schema" => {
