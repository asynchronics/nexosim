...
// List of keywords that can appear in #[serde(...)]/#[schemars(...)] attributes
// which we want serde_derive_internals to parse for us.
pub(crate) static SERDE_KEYWORDS: &[&str] = &[
    ...
    // Message on remote types, but we parse that ourselves rather than using
...
// If a struct/variant/field has any #[schemars] attributes, then create copies
// of them as #[serde] attributes so that serde_derive_internals will parse them
// for us.
...
fn process_attrs(ctxt: &Ctxt, attrs: &mut Vec<Attribute>) {
    ...
    *attrs = other_attrs;
    ...
    for meta in get_meta_items(attrs, "schema", ctxt) {
    ...
    // Re-add #[serde(...)] attributes that weren't overridden by #[schemars(...)]
    // attributes
...
mod tests {
    ...
    fn test_process_serde_attrs() {
        let mut input: DeriveInput = parse_quote! {
            ...
            #[schema(rename = "overriden", another_unknown_word, !from)]
            #[misc]
            struct MyStruct {
                /// blah blah blah
