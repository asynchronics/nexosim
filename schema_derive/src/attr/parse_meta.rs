...
pub fn parse_contains(outer_meta: CustomMeta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    impl Parse for ContainsFormat {
        fn parse(input: ParseStream) -> syn::Result<Self> {
            // An imperfect but good-enough heuristic for determining whether it looks more
            // like a comma-separated meta list (validator-style), or a single
            // expression (garde-style). This heuristic may not generalise
            // well-enough for attributes other than `contains`! `foo = bar` =>
            // Metas (not Expr::Assign) `foo, bar`  => Metas
    ...
    let nested_meta_or_expr = match cx.attr_type {
        ...
        "schema" => parse_meta_list_with(&outer_meta, cx, ContainsFormat::parse),
        wat => {
...
// Like `parse_name_value_expr`, but if the result is a string literal, then
// parse its contents.
