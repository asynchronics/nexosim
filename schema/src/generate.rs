...
pub struct SchemaSettings {
    ...
    ///
    /// This setting is respected by `#[derive(Message)]` on enums, but manual implementations
    /// of `Message` may ignore this setting.
    ///
    /// Defaults to `false`.
...
/// # Example
/// ```
/// use schemars::{Message, SchemaGenerator};
///
/// #[derive(Message)]
/// struct MyStruct {
///     foo: i32,
...
impl SchemaGenerator {
    ...
    ///
    /// If `T` is not [inlined](Message::inline_schema), this will add `T`'s schema to
    /// this generator's definitions, and return a `$ref` schema referencing that schema.
    /// Otherwise, this method behaves identically to [`Message::json_schema`].
    ///
    /// If `T`'s schema depends on any [non-inlined](Message::inline_schema) schemas, then
    /// this method will add them to the `SchemaGenerator`'s schema definitions.
    pub fn subschema_for<T: ?Sized + Message>(&mut self) -> Schema {
        struct FindRef {
            schema: Schema,
    ...
    fn insert_new_subschema_for<T: ?Sized + Message>(&mut self, name: CowStr, uid: &SchemaUid) {
    ...
    /// Borrows the collection of all [non-inlined](Message::inline_schema) schemas that
    /// have been generated.
    ///
    /// The keys of the returned `Map` are the [schema names](Message::schema_name), and the
    /// values are the schemas themselves.
    pub fn definitions(&self) -> &JsonMap<String, Value> {
        &self.definitions
    }

    /// Mutably borrows the collection of all [non-inlined](Message::inline_schema)
    ...
    /// Returns the collection of all [non-inlined](Message::inline_schema) schemas that
    ...
    pub fn root_schema_for<T: ?Sized + Message>(&mut self) -> Schema {
    ...
    /// this method will include them in the returned `Schema` at the [definitions
    /// path](SchemaSettings::definitions_path) (by default `"$defs"`).
    pub fn into_root_schema_for<T: ?Sized + Message>(mut self) -> Schema {
        let schema_uid = self.schema_uid::<T>();
        self.root_schema_id_stack.push(schema_uid.clone());
    ...
    ///
    /// If the value implements [`Message`], then prefer using the
    ...
    fn json_schema_internal<T: ?Sized + Message>(&mut self, uid: &SchemaUid) -> Schema {
    ...
    fn schema_uid<T: ?Sized + Message>(&self) -> SchemaUid {
