...
use crate::{Message, Schema, SchemaGenerator};
...
pub fn json_schema_for_internally_tagged_enum_newtype_variant<T: ?Sized + Message>(
...
pub fn json_schema_for_flatten<T: ?Sized + Message>(
    generator: &mut SchemaGenerator,
    required: bool,
) -> Schema {
...
/// Hack to simulate specialization:
/// `<MaybeMessageWrapper<T>>::maybe_schema_id()` will resolve to either
/// - The inherent method `MaybeMessageWrapper::maybe_schema_id()` if T impls `Message`
///     - this returns `T::schema_id()`
/// - The trait method `NoMessage::maybe_schema_id()` from the blanket impl otherwise
...
macro_rules! _schemars_maybe_schema_id {
    ($ty:ty) => {{
        #[allow(unused_imports)]
        use $crate::_private::{MaybeMessageWrapper, NoMessage as _};

        <MaybeMessageWrapper<$ty>>::maybe_schema_id()
    }};
}

pub struct MaybeMessageWrapper<T: ?Sized>(core::marker::PhantomData<T>);
...
impl<T: ?Sized> NoMessage for T {}

impl<T: Message + ?Sized> MaybeMessageWrapper<T> {
