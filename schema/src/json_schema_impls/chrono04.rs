...
macro_rules! formatted_string_impl {
    ($ty:ident, $format:literal) => {
        formatted_string_impl!($ty, $format, Message for $ty);
    };
...
formatted_string_impl!(DateTime, "date-time", <Tz: TimeZone> Message for DateTime<Tz>);
