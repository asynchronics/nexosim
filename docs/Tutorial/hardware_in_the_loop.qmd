...
returned, indicating there isn't anymore data to read. Other errors cause the
loop to exit.
...
able to send data to the serial port. You might've already noticed the problem
we're facing. NeXosim models must be serializable, but we won't be able to
serialize `IoThread`.
...
use my_serial_port::{ProtoSerialPort, SerialPort};
