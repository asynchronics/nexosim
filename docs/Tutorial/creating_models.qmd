...
[`ReplierFn`][ReplierFn] trait. The trait is implemented for model asynchronous
methods with a mutable self reference, and optionally, an argument with a type
implementing `Clone` and `Send` and an argument that is a mutable reference to
the model's `Context`. The returned future must implement `Send`.
...
### Model builders and initializers

#### Model prototypes

The models we use in this tutorial are rather simple, and don't require more
complicated logic during construction. Models for which this is not the case
can be constructed by a model prototype. Model prototypes are types that
implement the `ProtoModel` trait.

Let's say we want to add a `Counter` component to the SunSensor model that
counts and broadcasts the number of sample commands the sensor has received.

```{.rust source-line-numbers="1-11,14,15,17"}
struct Counter {
    out: Output<u32>,
    count: u32,
}
...
impl Counter {
    async fn increment(&mut self) {
        self.count += 1;
        self.out.send(self.count).await;
    }
}
...
The sun sensor model prototype `ProtoSunSensor` has to expose the output and
requestor ports of both the model and the counter, so that the ports can be
connected to.

```rust
pub struct ProtoSunSensor {
    pub sample_count: Output<u32>,
    pub tm: Output<SampleReply>,
    pub requestor: UniRequestor<(), [f32; 3]>,
    address: u8
}
...
impl ProtoSunSensor {
    pub fn new(address: u8, sensor_req: UniRequestor<(), [f32; 3]>) -> Self {
        Self {
            sample_count: Output::default(),
            out: Output::default(),
            requestor: sensor_req,
            address,
        }
    }
}
...
The `ProtoModel` trait requires an implementation of the `build` method. Here
we create a `Counter` instance where we move the `sample_count` output to.
In turn, the counter is moved to the new `SunSensor` along with the `out` output
and the `requestor` UniRequestor.
...
The `build` method is called by the simulation builder when the model prototype
is added to the simulation.

Note that now the `SunSensor` output and requestor ports no longer need to be
public.

```{.rust source-line-numbers="2,3"}
pub struct SunSensor {
    out: Output<SampleReply>,
    requestor: UniRequestor<(), [f32; 3]>,
    voltage: f32,
    counter: Counter,
}
```

The model prototype can be used the same way we use regular models. In `main.rs`
we can replace:
...
The `Model` trait defines the `init` method, which is called once, when the
simulation is initialized using the `SimInit::init` method. At this point
the connections between models are established. This means the method can be
used to broadcast initial events or let the model schedule events for itself
using the `Context` object.

------

We've defined the models, how they behave upon receiving an event, and what
events they can broadcast. The next step is to set up how the models interact
with each other, which we'll go over in the [bench assembly](bench_assembly.qmd)
tutorial.
