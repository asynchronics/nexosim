...
There are also query sources. A `QuerySource` can be used the same way as an
an `EventSource`, but can be used to retrieve replies from all connected
replier ports.

## Server

NeXosim includes a gRPC server that allows you to control the simulation using
[`NeXosim-py`](https://github.com/asynchronics/nexosim-py) - the Python client
...
fn simulation(cfg: Config) -> Result<(Simulation, EndpointRegistry), SimulationError> {
    ...
    let mut sensor = sensor::SunSensor::new(cfg.sensor_address, requestor);
    let sensor_mbox = Mailbox::new();
